...
class EF4(Function):
    ...
    def get_electric(self):
        ...
        Step = (self.Step1, self.Step2, self.Step3, self.Step4)
        for n, c in enumerate(self.tests):
            if Step[n] != 'OFF':
                ...
                self.te.graph_vector([(c[4], f13i0, 'F1'), #F1
                                      ...
                                      (c[4], 0, 'Uc=3U0')], title=f'{n+1} ступень НТЗНП')
    ...
    def __str__(self):
        return f'Step1 {self.Step1}, IN1> {self.IN1}%, t1 {self.t1}c, Step2 {self.Step2}, IN2> {self.IN2}%, t2 {self.t2}c, ' \
               f'Step3 {self.Step3}, IN3> {self.IN3}%, t3 {self.t3}c, Step4 {self.Step4}, Characteristic {self.Characteristic},' \
               f' IN>inv {self.INgtinv}, k {self.k}, IN4 {self.IN4}%, t4 {self.t4}c, IN>Dir {self.INDir}%'
